...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function init() {
        ...
        const materiaKey = params.get('materia') || 'sociales'; // Default por si acaso
        const title = materias[materiaKey] || 'Simulador';
        
        // 1. Establecer Títulos y Datos Visuales INMEDIATAMENTE
        document.getElementById('titulo-materia').textContent = title.toUpperCase();
        if(txtMateria) txtMateria.textContent = title;
        
        // 2. Configurar Reglas (Tiempo/Preguntas)
        if(materiaKey.includes('matematicas')) {
            ...
            timeLeft = 3600; // 60 min (Default para Sociales ESMIL)
            totalPreguntas = 50;
        }

        // Actualizar UI con datos base (mientras carga)
        ...
        try {
            ...
            // Actualizar número real de preguntas cargadas
